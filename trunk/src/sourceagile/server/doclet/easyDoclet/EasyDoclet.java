package sourceagile.server.doclet.easyDoclet;
...
public class EasyDoclet {
	...
	protected EasyDoclet(File sourceDirectory, String[] packageNames,
			File[] fileNames) {
		...
		for (File fileName : fileNames) {
			// log.fine("Adding file to documentation path: "
		...
		for (String packageName : packageNames) {
			// log.fine("Adding sub-packages to documentation path: "
			// + packageName);
			subPackages.append(packageName);
		}

		new PublicMessager(context, getApplicationName(), new PrintWriter(
				new LogWriter(Level.SEVERE), true), new PrintWriter(
				new LogWriter(Level.WARNING), true), new PrintWriter(
		...
		try {
			rootDoc = javadocTool.getRootDocImpl("", null, new ModifierFilter(
					ModifierFilter.ALL_ACCESS), javaNames.toList(),
					new ListBuffer<String[]>().toList(), false, subPackages
							.toList(), new ListBuffer<String>().toList(),
					false, false, false);
		} catch (Exception ex) {
